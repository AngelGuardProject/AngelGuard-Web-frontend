...
function WriteBoard() {
    ...
    return (
        <div>
            <Header color='white' />
            <div className={style.thumbnail} style={{ backgroundImage: `url(${thumbnailImage})` }}>
                <img src={cameraIcon} alt='Change profile' className={style.cameraIcon} onClick={handleImageClick} />
                <input id='image' type='file' accept='image/*' onChange={handleImageUpload} ref={fileInputRef} style={{ display: "none" }} />
            ...
            <div className={style.btnBox}>
                <button className={`${style.btn} ${style.cancel}`} onClick={handleCancel}>
                    취소
                </button>
                <button className={`${style.btn} ${style.submit}`} onClick={uploadDataToServer}>
                    완료
                </button>
            </div>
        </div>
