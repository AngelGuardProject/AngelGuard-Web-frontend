...
function BabyInfo() {
  ...
  const fetchBabies = () => {
    axios
      .get(`http://34.47.76.73:3000/mypage/baby/${user_login_id}`)
      .then((res) => {
  ...
  const handleUpdate = (updatedData) => {
    const updatedBabies = babies.filter(
      (b) => b.baby_id !== updatedData.baby_id
    ); // 삭제된 아기 정보를 제외하고 필터링
  ...
  return (
    <div>
      {showBabyPlus && (
        <BabyPlus onCancel={handleCancel} onAdd={handleAddBaby} />
      )}
      {selectedBaby && (
        <BabyEdit
          baby={selectedBaby}
        ...
        <div className={style.BabyContainer}>
          ...
          <div className={style.babyList}>
            {babies.map((baby) => (
              <div
                key={baby.baby_id}
                className={style.babyInfo}
                onClick={() => handleBabyClick(baby)}
              >
                <div className={style.babyImgBox}>
                  <img
                    className={`${style.babyImg} ${
                      baby.baby_sex !== "female" ? style.boy : ""
                    }`}
                    src={require(`../../assets/${
                      baby.baby_sex === "female" ? "girl" : "boy"
                    }.png`)}
                    alt="Baby"
                  />
                </div>
                ...
                <div className={style.babyBirth}>
                  {baby.baby_sex === "female" ? "여아" : "남아"} /{" "}
                  {calculateAge(baby.baby_birth)}세
                </div>
              </div>
            ))}
          </div>
