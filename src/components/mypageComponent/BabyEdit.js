...
function BabyEdit({ baby, onCancel, onUpdate }) {
  ...
  const handleDeleteBaby = () => {
    ...
    axios
      .post(
        `http://l34.47.76.73:3000/mypage/babydelete/${user_login_id}/${baby.baby_id}`
      )
      .then((res) => {
  ...
  const handleEditBaby = () => {
    ...
    const user_login_id = localStorage.getItem("user_login_id");
    const newBirthdate = `${birthYear}-${birthMonth.padStart(
      2,
      "0"
    ...
    axios
      .post(
        `http://34.47.76.73:3000/mypage/babyupdate/${user_login_id}/${baby.baby_id}`,
        updatedData
      )
      .then((res) => {
