...
function MyInfo() {
  ...
  useEffect(() => {
    axios
      .get(`http://34.47.76.73:3000/user/myprofile/${user_login_id}`, {
        headers: {
    ...
    axios
      .post(`http://34.47.76.73:3000/image/porfile/${user_login_id}`, null, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const handleImageUpload = (event) => {
    ...
    if (file) {
      ...
      axios
        .post(
          `http://34.47.76.73:3000/image/upload/${user_login_id}`,
          formData,
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  const handleUpdateUserInfo = () => {
    ...
    axios
      .post(
        `http://34.47.76.73:3000/user/update/${user_login_id}`,
        updatedUserInfo,
        {
          headers: {
            Authorization: `Bearer ${token}`,
